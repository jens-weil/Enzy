...
export async function postToInstagram(data: {
  ...
  let { accountId, accessToken } = getInstagramSettings();
  
  accessToken = accessToken?.trim();

  if (!accountId || !accessToken) {
    const errorMsg = `Inställningar saknas: Account ID (${!!accountId}) eller Access Token (${!!accessToken})`;
  ...
  try {
    ...
    if (!containerResponse.ok) {
      const igErr = containerResult.error;
      const detailedError = igErr ? `${igErr.message} (Code: ${igErr.code}, Subcode: ${igErr.error_subcode})` : "Failed to create media container";
    ...
    if (!publishResponse.ok) {
       const igErr = publishResult.error;
       const detailedError = igErr ? `${igErr.message} (Code: ${igErr.code}, Subcode: ${igErr.error_subcode})` : "Failed to publish media container";
       throw new Error(detailedError);
    }
